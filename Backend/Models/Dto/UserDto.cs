...
namespace Backend.Models.Dto
{
    public abstract class UserDto
    {
        ...
        [Required]
        [StringLength(18, MinimumLength = 10, ErrorMessage = "Длина номера телефона 10-18 символов")]
        [RegularExpression(
        @"^\s*(\+7|7|8)?[\s-]?\(?[0-9]{3}\)?[\s-]?[0-9]{3}[\s-]?[0-9]{2}[\s-]?[0-9]{2}\s*$", ErrorMessage = "Неверный формат телефона. Используйте российский номер")]
